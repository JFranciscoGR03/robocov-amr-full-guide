...
class OdometryNode(Node):
    ...
    #def imu_callback(self, msg):
        #self.imu_orientation_q = msg.orientation

    def reset_callback_from_initialpose(self, msg):
        ...
        self.get_logger().info(f"Odometry reset via /initialpose to x={self.old_x:.2f}, y={self.old_y:.2f}, yaw={self.old_theta:.2f}")
    ...
    def timer_callback(self):
        ...
        '''if self.imu_orientation_q is not None:
            theta = self.get_yaw_from_quaternion(self.imu_orientation_q) % (2 * np.pi)
        else:
            theta = self.old_theta % (2 * np.pi)'''
        ...
        # Actualizar posición angular de las ruedas
        ...
        self.get_logger().info(f'Pos x: {x:.2f} y: {y:.2f} θ: {theta:.2f} Dist: {distance:.2f}')
